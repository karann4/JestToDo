...
it('DELETE by id' + endpointUrl+ ":todoId", async()=>{
    const response =  await request(app)
        .delete(endpointUrl + fisrtTodo._id)

    expect(response.statusCode).toBe(200);

});

it('should return error 404 on no data with DELETE' + endpointUrl+ "5f281c93b9308e43044c899d", async()=>{
    const response =  await request(app)
        .delete(endpointUrl)
        .send({message:"Error"});
...
it("POST update by Id" + endpointUrl + ":todoId", async()=>{
    const response = await request(app).post(endpointUrl + fisrtTodo._id).send(newTodo)
    expect(response.statusCode).toBe(200);
    expect(response.body.title).toBe(fisrtTodo.title);
    expect(response.body.done).toBe(fisrtTodo.done);
});

it("'should return error 404 on data not found" + endpointUrl + "5f281c93b9308e43044c899d", async()=>{
